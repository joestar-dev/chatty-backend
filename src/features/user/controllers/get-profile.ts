...
export class Get {
  ...
  public async profileAndPosts(req: Request, res: Response): Promise<void> {
    ...
    const cachedUser: IUserDocument = await userCache.getUserFromCache(userId) as IUserDocument;
    const cachedUserPost: IPostDocument[] = await postCache.getUserPostsFromCache('post', parseInt(uId, 10)) as IPostDocument[];

    const existingUser: IUserDocument = cachedUser ? cachedUser : await userService.getUserById(userId);

    const userPosts: IPostDocument[] = cachedUserPost.length
      ? cachedUserPost
  ...
  private async followers(userId: string): Promise<IFollowerData[]> {
    const cachedFollower: IFollowerData[] = await followerCache.getFollowersFromCache(`followers:${userId}`);
    const result = cachedFollower.length ? cachedFollower : await followerService.getFollowerData(new mongoose.Types.ObjectId(userId));
