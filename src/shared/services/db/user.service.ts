...
class UserService {
  async addUserData(data: IUserDocument): Promise<void> {
  ...
  public async searchUsers(regex: RegExp): Promise<ISearchUser[]> {
    const users = await AuthModel.aggregate([
      ...
      { $lookup: { from: 'User', localField: '_id', foreignField: 'authId', as: 'user' } }, // localField from AuthModel - foreignField from User(User collection)
      { $unwind: '$user' },
      {
        $project: {
          _id: '$user._id', // _id from User collection
          username: 1,
          email: 1,
          avatarColor: 1,
          profilePicture: 1
