...
class ReactionService {
  public async addReactionDataToDB(reactionData: IReactionJob): Promise<void> {
    ...
    const updatedReaction: [IUserDocument, IReactionDocument, IPostDocument] = await Promise.all([
      ...
      ReactionModel.replaceOne({ postId: postId, type: previousReaction, username: username }, updatedReactionObject, { upsert: true }),
    ...
    if (updatedReaction[0].notifications.reactions && userTo !== userFrom) {
      ...
      emailQueue.addEmailJob('reactionsEmail', { receiverEmail: updatedReaction[0].email!, subject: `Post reaction notification.`, template})
    }
  }
  ...
  public async removeReactionDataFromDB(reactionData: IReactionJob): Promise<void> {
    ...
    Promise.all([
      ReactionModel.deleteOne({ postId: postId, type: previousReaction, username: username }),
      PostModel.updateOne(
        { _id: postId },
        {
          $inc: {
